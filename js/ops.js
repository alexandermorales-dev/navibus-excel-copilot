...
const Ops = {
  ...
  async execute(ops, opts = {}) {
    ...
    for (const op of ops) {
      ...
      console.log(`Ops: recipe ${op.op} input:`, JSON.stringify({ sheet: op.sheet, source: op.source, kpis: op.kpis, groupBy: op.groupBy, valueColumn: op.valueColumn, agg: op.agg, charts: op.charts }));
      console.log(`Ops: recipe ${op.op} expansion:`, expansion.ok ? `ok (${expansion.ops?.length} ops)` : `FAILED: ${expansion.error}`);
      if (expansion.ok && expansion.warnings?.length > 0) {
        console.log(`Ops: recipe warnings:`, expansion.warnings);
      }
      if (expansion.ok) {
        console.log(`Ops: recipe ops detail:`, expansion.ops.map(o => `${o.op} ${o.range || o.name || ''}`));
      }
      if (!expansion.ok) {
